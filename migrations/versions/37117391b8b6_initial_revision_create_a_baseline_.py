...
def upgrade() -> None:
    ...
    op.create_table('contact_document',
    ...
    sa.Column('password_expires', sa.DateTime(timezone=True), server_default=sa.text("now() + interval '1 year'"), nullable=False),
    ...
    op.create_index(op.f('ix_title_name'), 'title', ['name'], unique=False)
    op.create_table('user_contact_document',
    sa.Column('user_id', sa.BigInteger(), nullable=False),
    sa.Column('contdoc_id', sa.BigInteger(), nullable=False),
    sa.Column('name', sa.String(length=128), nullable=False),
    sa.Column('is_main', sa.Boolean(), server_default='f', nullable=False),
    sa.ForeignKeyConstraint(['contdoc_id'], ['contact_document.id'], ),
    sa.ForeignKeyConstraint(['user_id'], ['users.id'], ),
    sa.PrimaryKeyConstraint('user_id', 'contdoc_id', 'name')
    )
    op.create_index(op.f('ix_user_contact_document_name'), 'user_contact_document', ['name'], unique=False)
    ...
    sa.Column('time_created', sa.DateTime(timezone=True), server_default=sa.text('now()'), nullable=False),
    sa.Column('time_updated', sa.DateTime(timezone=True), nullable=True),
    sa.ForeignKeyConstraint(['food_component_id'], ['food_component.id'], ),
    sa.ForeignKeyConstraint(['food_id'], ['food.id'], ),
    sa.PrimaryKeyConstraint('id')
    )
    op.create_index(op.f('ix_food_composition_food_component_id'), 'food_composition', ['food_component_id'], unique=False)
    op.create_index(op.f('ix_food_composition_food_id'), 'food_composition', ['food_id'], unique=False)
    op.create_index(op.f('ix_food_composition_name'), 'food_composition', ['name'], unique=False)
    op.create_table('last_used_password',
    sa.Column('user_id', sa.BigInteger(), nullable=False),
    sa.Column('password', sa.String(length=64), nullable=False),
    sa.Column('time_created', sa.DateTime(timezone=True), server_default=sa.text('now()'), nullable=False),
...
def downgrade() -> None:
    ...
    op.drop_index(op.f('ix_food_composition_name'), table_name='food_composition')
